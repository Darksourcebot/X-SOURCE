...
def main_menu():
    text = f"**- [سـورس إكس](https://t.me/SOURCEE_X)\
        \n المسـاعد\
        \n\
        \n◽  إكس لـ {mention}**"
    buttons = [
...
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("l313l"):
            buttons = [
                (
                    ...
                    Button.url(" HuRe UsᴇʀBoᴛ", "https://t.me/SOURCEE_X"),
                )
            ]
            ...
            result = builder.article(
                title="رسـالة تـرول من سـورس  إكس 🧸♥",
            ...
            result = builder.article(
                title="همسـة سـرية من سـورس  إكس 🧸♥",
                text=f"فقـط {razan} هـو مـن يستـطيع مشـاهدتهـا !",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            ...
            result = builder.article(
                title="رسـالة مخفيـة من سـورس  إكس 🧸♥",
                text=f"✖✖✖",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
        ...
        buttons = [
            (
                Button.url("قنـاة السـورس ⚒️", "https://t.me/SOURCEE_X"),
                Button.url(
                    "كـروب المـساعدة 📬",
                    "https://t.me/p_ouv",
                ),
        ...
        text, msg_entities = await event.client._parse_message_text(
            "**[بـوت  إكس 🧸♥](https://t.me/SOURCEE_X)**\
            \n\
            \n❤  إكس هـو بـوت بسـيط يدخـل الـى حسـابك لجعـلك تتحـكم به معـ اوامـر معينـة.\
            \n\
            \n**اذا كـنت تـرغب بتـنصيـب السـورس عـلى حسـابك ,\
            \n🐾 تعال الـى [كروب المسـاعدة](https://t.me/p_ouv)!**",
            "md",
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
