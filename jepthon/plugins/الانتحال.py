...
DEFAULTUSERBIO = (
    str(DEFAULT_BIO)
    if DEFAULT_BIO
    else "الحمد لله دائماً وابداً 🎀 𝑆𝐻 : @SOURCEE_X"
)
...
async def reda(event):
    if event.is_group or event.is_channel:
        ...
        if len(messageo) != 8:
            return await edit_delete(event, "لا تغير الرسالة @SOURCEE_X")
        if BOTLOG:
            await event.client.send_message(
                BOTLOG_CHATID,
